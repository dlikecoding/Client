...
import { FilterIcon, SortAscIcon, SortDescIcon, ZoomInIcon, ZoomOutIcon } from "../../svgIcons";
...
const FilterPhotoView = () => {
  ...
  return (
    <Show when={!isSelected()}>
      <button popovertarget="filter-timeline">{FilterIcon()}</button>
      <div popover="auto" id="filter-timeline" class="popover-container filter_timeline">
        ...
        <For each={sortOptions}>
          {({ type, label }) => (
            <div
              class={`sort_filter ${params.sortKey === type ? "activeFilter" : ""}`}
...
export default FilterPhotoView;
