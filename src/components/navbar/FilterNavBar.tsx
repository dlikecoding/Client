...
const FilterTimeline = () => {
  // const { view, setView, updatePage, updatePageKey, isAll } = useManageURLContext();
  ...
  // // Effect: Reset filter on route change
  // createEffect(() => {
  //   if (!isAll()) setSelectedType("");
  // });
  ...
  // const changeGrid = () => {
  //   setView("objectFit", (prev: number) => !prev);
  // };
  ...
  return (
    <div popover="auto" id="filter-timeline" class="popover-container filter_timeline">
      ...
      <For each={sortOptions}>
        {({ type, label }) => (
          <div
            class={`sort_filter ${sortData.type === type ? "active" : ""}`}
